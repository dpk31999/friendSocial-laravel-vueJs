    ...
    <div class="row justify-content-center info">
      ...
      <div class="col-xl-6 col-8">
        <div class="d-flex mb-3">
          ...
          <div v-else>
            <button v-if="statusFriend == ''" :class="{ 'btn-loading' : isLoading == true }" class="btn btn-secondary" @click="handleFriend(userCurrent.id)">Thêm bạn bè</button>
            <button v-else-if="statusFriend == '0'" :class="{ 'btn-loading' : isLoading == true }" class="btn btn-info" @click="handleFriend(userCurrent.id)">Hủy lời mời</button>
            <button v-else-if="statusFriend == 'special'" :class="{ 'btn-loading' : isLoading == true }" class="btn btn-info" @click="handleFriend(userCurrent.id)">Chấp nhận kết bạn</button>
            <button v-else-if="statusFriend == '1'" :class="{ 'btn-loading' : isLoading == true }" class="btn btn-primary" @click="handleFriend(userCurrent.id)">Hủy kết bạn</button>
          </div>
        </div>
        <div class="d-flex mb-3">
          <span class="mr-4">
    ...
    <div class="row justify-content-center">
      <div class="col-md-4">
          ...
          <div class="text-information">
            <span>
              <img
                ...
                alt
              /> Sinh ngày
              <span class="font-weight-bold">{{ userCurrent.date_of_birth }}</span>
            </span>
          </div>
          <div class="text-information">
            <span>
        ...
        <div class="left-banner mb-3">
          ...
          <div class="text-information">
            <span>
              <img class="icon" src="https://img.icons8.com/wired/64/000000/email.png" /> Email
              <span class="font-weight-bold">{{ userCurrent.email }}</span>
            </span>
          </div>
          <div class="text-information">
            <span>
              <img class="icon" src="https://img.icons8.com/material-outlined/48/000000/phone.png" /> Số điện thoại
              <span class="font-weight-bold">{{userCurrent.phone}}</span>
            </span>
          </div>
          <div class="text-information">
            <span>
              <img class="icon" src="https://img.icons8.com/metro/26/000000/facebook-new.png" /> Facebook
              <span class="font-weight-bold"><a :href="userCurrent.link_fb" target="_blank">{{userCurrent.name}}</a></span>
            </span>
          </div>
        </div>
        ...
        <div class="left-banner">
          ...
          <div class="interests" v-for="interest in interests" :key="interest.id">
      ...
      <div class="col-md-7">
          ...
          >
            ...
            <div class="form-group">
              <label for="file-input">
                ...
                >
                  <span>
                    <img src="https://img.icons8.com/plasticine/100/000000/image.png" width="30px" />
                  </span>Chọn ảnh
                </label>
              </label>
              <img v-if="urlImage" :src="urlImage" width="100" height="100" />
              <input type="file" ref="file" id="imagePost" class="d-none" @change="onFileImage" />
        ...
        <div class="list-post" v-if="posts.length > 0">
          <div v-for="(post,index) in posts" :key="post.id" class="left-banner mb-3">
            ...
            <div v-else class="d-flex justify-content-between" style="padding: 10px">
              <input type="text" class="form-control" v-model="selectedPost.title">
              <button class="btn btn-light" @click="updatePost(index)">Sửa</button>
            </div>
...
export default {
  ...
  methods: {
    ...
    async getListPostCurrent() {
      await axios.get("/api/posts/" + this.$route.params.username) .then((response) => {
    ...
    updatePost(index){
      axios.put('/api/posts/' + this.selectedPost.id, {title: this.selectedPost.title})
				.then(response => {
    ...
    {
      await axios.get("/api/friends/status/" + this.$route.params.username)
      .then((response) => {
        this.statusFriend = response.data.status
      })
      .catch((error) => {
    ...
    {
      await axios.get("/api/friends/count/" + this.$route.params.username)
      .then((response) => {
